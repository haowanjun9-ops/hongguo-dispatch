...
async def extract_all_pages():
    ...
    try:
        ...
        async with websockets.connect(ws_url, max_size=10*1024*1024) as websocket:
            ...
            while True:
                ...
                if 'result' in result and 'result' in result['result']:
                    ...
                    page_data = value.get('data', []) if isinstance(value, dict) else []
                    ...
                    if isinstance(page_data, list) and len(page_data) > 0:
                        ...
                        print(f"警告: 第 {current_page} 页没有提取到数据", flush=True)
                else:
                    print(f"警告: 第 {current_page} 页提取失败", flush=True)
                    if 'exceptionDetails' in result:
                        print(f"异常: {result['exceptionDetails']}", flush=True)

                # 检查是否到达最后一页
                if current_page >= total_pages:
                    print("已处理完所有页面", flush=True)
                    break
                ...
                if next_result.get('result', {}).get('result', {}).get('value') == True:
                    # 等待页面加载
                    await cdp.evaluate_script(websocket, wait_script)
                    current_page += 1
                else:
                    print("没有更多页面了", flush=True)
                    break
        ...
        for item in all_data:
            try:
                cost_str = item['消耗'].replace('¥', '').replace(',', '').replace('元', '').strip()
                cost = float(cost_str)
                if cost > 2000:
                    item['消耗值'] = cost
        ...
        print(f"筛选出 {len(filtered_data)} 条消耗>2000的记录", flush=True)
