...
class CDPClient:
    ...
    async def navigate(self, websocket, url):
        """导航到指定URL"""
        print(f"正在导航到: {url}")
        result = await self.send_command(websocket, "Page.navigate", {"url": url})
        return result

    async def wait_for_load(self, websocket, timeout=30):
        """等待页面加载完成"""
    ...
    async def evaluate_script(self, websocket, script):
        ...
        return await self.send_command(
            websocket,
            "Runtime.evaluate",
            {"expression": script, "returnByValue": True, "timeout": 60000}
        )
...
async def extract_star_report():
    ...
    try:
        ...
        print(f"连接到页面: {target_page.get('url', 'unknown')}")
        ...
        async with websockets.connect(ws_url) as websocket:
            ...
            await cdp.navigate(websocket, target_url)
            await cdp.wait_for_load(websocket)
            ...
            while True:
                ...
                if next_result.get('result', {}).get('value') == True:
                    # 等待页面加载
...
if __name__ == "__main__":
    ...
    if result[0]:
        print(f"\n成功! CSV文件: {result[0]}, 筛选记录数: {result[1]}")