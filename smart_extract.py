...
class CDPClient:
    ...
    async def evaluate_script(self, websocket, script, await_promise=False):
        """执行JavaScript脚本并返回结果"""
    ...
    async def navigate_to(self, websocket, url):
        """导航到指定URL"""
        print(f"正在导航到: {url}", flush=True)
        result = await self.send_command(websocket, "Page.navigate", {"url": url})
        return result
...
async def extract_all_pages():
    ...
    try:
        ...
        current_url = pages[0].get('url', 'unknown')
        print(f"连接到浏览器: {current_url}\n")
        ...
        async with websockets.connect(ws_url, max_size=10*1024*1024) as websocket:
            ...
            # 如果不是目标页面,先导航
            target_url = "https://apex.whaleidea.cn/star/report"
            if 'apex.whaleidea.cn/star/report' not in current_url:
                print(f"当前页面不是星广报表页面，正在导航...", flush=True)
                await cdp.navigate_to(websocket, target_url)
                # 等待页面加载
                await asyncio.sleep(5)
                print("✅ 页面已导航\n", flush=True)

            # 检查页面状态
            check_url_script = "window.location.href"
            url_result = await cdp.evaluate_script(websocket, check_url_script)
            current_page_url = url_result.get('result', {}).get('result', {}).get('value', 'unknown')
            print(f"当前页面URL: {current_page_url}\n", flush=True)
            ...
            (function() {
                ...
                const loginText = document.body.innerText.includes('登录') || document.body.innerText.includes('login');
                return {
            ...
            """
            login_result = await cdp.evaluate_script(websocket, check_login_script)
            login_info = login_result.get('result', {}).get('result', {}).get('value', {})

            if login_info.get('hasLoginForm') or login_info.get('hasLoginBtn') or login_info.get('hasLoginText'):
                print("⚠️ 检测到可能需要登录", flush=True)
                print("请在浏览器中手动登录，然后按回车继续...", flush=True)
                # 等待用户登录（在实际环境中这里会等待）
            ...
            """
            table_result = await cdp.evaluate_script(websocket, check_table_script)
            table_info = table_result.get('result', {}).get('result', {}).get('value', {})
            print(f"表格状态: {json.dumps(table_info, ensure_ascii=False)}\n", flush=True)

            if not table_info.get('hasTable'):
                print("❌ 没有找到表格，可能需要更长的等待时间或页面未正确加载", flush=True)
                # 尝试等待更长时间
                for i in range(5):
                    print(f"等待数据加载... {i+1}/5", flush=True)
                    await asyncio.sleep(2)
                    table_result = await cdp.evaluate_script(websocket, check_table_script)
                    table_info = table_result.get('result', {}).get('result', {}).get('value', {})
                    if table_info.get('rowCount', 0) > 0:
                        print(f"✅ 检测到 {table_info['rowCount']} 行数据", flush=True)
                        break
